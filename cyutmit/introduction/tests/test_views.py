...
class introductionViewTestCase(TestCase):
    ...
    def test管理者新增消息表單空白標題或內容會顯示錯誤訊息(self):
        self.client.login(username='admin', password='admin')
        response = self.client.post(reverse('introduction:addIntroduction'), data={'title':'標題'})
        self.assertIn('必填', response.content.decode('utf-8'), 1)
        response = self.client.post(reverse('introduction:addIntroduction'), data={'content':'內容'})
        self.assertIn('必填', response.content.decode('utf-8'), 1)
        response = self.client.post(reverse('introduction:addIntroduction'))
        self.assertIn('必填', response.content.decode('utf-8'), 2)


    def test管理者新增消息表單可正確儲存消息(self):
        self.client.login(username='admin', password='admin')
        response = self.client.post(reverse('introduction:addIntroduction'), data={'title':'title', 'content':'content'})
